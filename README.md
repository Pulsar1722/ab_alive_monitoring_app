...
### 3.1. Webページ死活監視  
「Webページ死活監視」は、監視対象のWebページに対して死活監視を行い、その応答を確認する機能である。「Webページ死活監視」は、本アプリを実行する毎に実施する。「Webページ死活監視」の手順を以下に示す。  
**※定期的に実行するには、別途`cron`コマンド等を用いること**
...
## 4.1. 参考: `cron`の設定・使用方法
死活監視アプリを定期実行するための`cron`コマンドの設定・使用方法を以下に示す。  
